...
const Masonry: React.FC<MasonryProps> = ({
  ...
  const grid = useMemo<GridItem[]>(() => {
    ...
    // First pass: calculate positions and find max height
    const itemsWithPositions = items.map((child) => {
    ...
    // Second pass: adjust y positions to align all columns to the same height
    return itemsWithPositions.map((item) => {
      const currentColHeight = colHeights[item.col];
      const adjustedY = item.y + (maxHeight - currentColHeight);
      return { ...item, y: adjustedY };
    });
  }, [columns, items, width]);
